    ...
    <section class="shell">
      ...
      <div class="card">
        ...
        <table *ngIf="employeeAssignments.length">
          ...
          <tbody><tr *ngFor="let item of employeeAssignments"><td>{{ item.employee?.id || selectedEmployeeId || 'desasignado' }}</td><td>{{ item.asset.assetCode }}</td><td>{{ item.asset.name }}</td><td>{{ item.asset.acquisitionCost }}</td><td>{{ item.assignedAt }}</td><td>{{ item.returnedAt || '-' }}</td><td>{{ item.asset.status }}</td></tr></tbody>
        </table>
      </div>
